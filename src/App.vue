<template>
  ...
  <nav class="navbar mynav">
    <div class="container-fluid">
      ...
      <table class="navbar-form navbar-right nav-table">
      <tr>
        <td>
          <form   role="search">
      ...
      <tr>
        <td>
          ...
          <div class="auth-form" v-if="is_login">
        ...
        <div  class="auth-form" v-if="is_login_check">
          <table class="is_check">
            <tr>
              ...
              <td>
                <button class="btn btn-auth" @click="logout" >Logout</button>
                <button class="btn btn-auth" @click="goToCart">
  ...
  <router-view :filterstring="search_param" @closeCart="closeCartEl" @clearSearch="clearSearchString"  @changeCountTotal="changeCart" :goCart="is_cart"/>
        ...
        axios.get('http://192.168.0.15/~user15/BOOK_SHOP/client/api/auth/'+localStorage['hash_log']+"/"+localStorage['user'], this.config)
            .then(function (response) {
              ...
              if(client_id>0){
                ...
                //axios.get('http://localhost:88/BOOK_SHOP/client/api/cart/'+ client_id, this.config)
                axios.get('http://192.168.0.15/~user15/BOOK_SHOP/client/api/cart/'+ client_id, self.config)
                .then(function (response) {
                  //console.log(response.data)
                  self.bag_book_count = parseInt(response.data.data)
              })
